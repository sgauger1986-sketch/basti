...
#   paris.mp4 (Akkordeonspieler in Montmartre, Kling 3.0; nur die Musik wird genutzt)
...
ffmpeg -v error -y -i toast.mp4 -i paris.mp4 -filter_complex \
  "[0:a]${AF}[a0];[1:a]${AF}[a1];[a0][a1]acrossfade=d=1.5[x];[x]atrim=0:10,${LN}[a]" -map "[a]" $A bed_photo.m4a
...
ffmpeg -v error -y -i amb1.mp4 -i paris.mp4 -i amb1.mp4 -filter_complex \
  "[0:a]${AF}[a0];[1:a]${AF}[a1];[2:a]${AF}[a2];[a0][a1]acrossfade=d=1.5[x];[x][a2]acrossfade=d=1.5[y];
   [y]atrim=0:16,${LN},afade=t=out:st=12:d=3[a]" -map "[a]" $A bed_end.m4a
...
ffmpeg -v error -y -loop 1 -i ov/photo_plate.png -loop 1 -i ov/ov_gradient.png -loop 1 -i ov/ov_photo.png -i bed_photo.m4a -filter_complex \
  "[0:v]$(kb 240)[p];[p][1:v]overlay=0:0:shortest=1[pg];
   [2:v]format=rgba,fade=t=in:st=1.0:d=1.2:alpha=1,fade=t=out:st=8.2:d=1.2:alpha=1[t];
   [pg][t]overlay=0:0:shortest=1[v]" \
  -map "[v]" -map 3:a -t 10 $V $A segP.mp4
...
ffmpeg -v error -y -loop 1 -i ov/end_card.png -vf "${NORM},fade=t=out:st=4:d=1" -t 5 $V segF_v.mp4
ffmpeg -v error -y -i segE_v.mp4 -i segF_v.mp4 -i bed_end.m4a -filter_complex \
...
read -r o1 o2 o3 o4 o5 < <(python3 -c "
import sys
d=[float(x) for x in sys.argv[1:]]; t=0; o=[]
for v in d:
    t+=v-0.8; o.append(t)
print(' '.join(f'{v:.3f}' for v in o))" "$(dur segA.mp4)" "$(dur segP.mp4)" "$(dur segB.mp4)" "$(dur segC.mp4)" "$(dur segD.mp4)")
echo "xfade offsets: $o1 $o2 $o3 $o4 $o5"
ffmpeg -v error -y -i segA.mp4 -i segP.mp4 -i segB.mp4 -i segC.mp4 -i segD.mp4 -i segEF.mp4 -filter_complex \
